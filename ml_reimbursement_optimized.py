...
class OptimizedReimbursementMLModel:
    def __init__(self):
        ...
        self.model_file = os.path.join(self.cache_dir, 'ml_models.pkl')
        self.scaler_file = os.path.join(self.cache_dir, 'ml_scaler.pkl')
    ...
    def engineer_features(self, X):
        ...
        # Special case indicators (from interview insights)
...
# Global model instance (trained once, cached for subsequent runs)
_model_instance = None

def get_optimized_model():
    global _model_instance
    if _model_instance is None:
        _model_instance = OptimizedReimbursementMLModel()
    return _model_instance

if __name__ == "__main__":
    if len(sys.argv) != 4:
        print("Usage: python3 ml_reimbursement_optimized.py <trip_duration_days> <miles_traveled> <total_receipts_amount>")
        sys.exit(1)
